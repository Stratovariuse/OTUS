...
-- Session #1
-- SELECT pg_backend_pid();
SELECT locktype, relation::REGCLASS, virtualxid AS virtxid, transactionid AS xid, mode, granted FROM pg_locks WHERE pid = 206;
...
-- Session #1
SELECT * FROM locks_v WHERE pid = 205;
...
-- Session #1
SELECT * FROM locks_v WHERE pid = 206;
...
-- Session #1
SELECT * FROM locks_v WHERE pid = 209;
...
BEGIN;
SELECT txid_current(), pg_backend_pid(); -- 474
...
BEGIN;
SELECT txid_current(), pg_backend_pid(); --206
SELECT * FROM accounts WHERE acc_no = 1 FOR SHARE;
